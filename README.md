...
`formatJson.rScript`|String|"$RSTATS_ROOT/bin/Rscript"|Path to Rscript
...
---|---|---|---
`resultZip`|File|All results from sequenza runs using gamma sweep.|vidarr_label: resultZip
`resultJson`|File?|Combined json file with ploidy and contamination data.|vidarr_label: resultJson
`gammaSummaryPlot`|File|png for summary plot showing the effect of different gamma values|vidarr_label: gammaSummaryPlot
`gammaMarkdownPdf`|File|rmarkdown pdf with all gamma-specific panels along with gamma effect summary plot|vidarr_label: gammaMarkdownPdf
